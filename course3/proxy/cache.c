...
void cache_entry_unref(cache_t *cache, cache_entry_t *entry) {
  ...
  if (ref_count <= 0 && !downloader_active && status != CACHE_ENTRY_COMPLETE) {
    ...
    if (entry->ref_count <= 0 && !entry->downloader_active &&
      ...
      size_t entry_size =
          sizeof(cache_entry_t) + strlen(entry->url) + 1 + entry->headers_len +
          (entry->total_size / CACHE_CHUNK_SIZE + 1) * sizeof(cache_chunk_t);
...
int cache_entry_set_headers(cache_entry_t *entry, const char *headers,
                            size_t len, int status_code,
...
int cache_entry_append_data(cache_t *cache, cache_entry_t *entry,
  ...
  while (offset < len) {
    ...
    size_t chunk_offset = chunk->size;
...
size_t cache_entry_read(cache_entry_t *entry, size_t offset, char *buf,
                        size_t buf_size) {
  pthread_mutex_lock(&entry->mutex);
  ...
  while (chunk && total_read < buf_size) {
    ...
    size_t to_read = (buf_size - total_read < available)
                         ? (buf_size - total_read)
                         : available;
...
int cache_entry_wait_data(cache_entry_t *entry, size_t offset, int timeout_ms) {
  ...
  while (entry->total_size <= offset && entry->status == CACHE_ENTRY_LOADING &&
    ...
    if (timeout_ms > 0) {
      ...
      int rc =
          pthread_cond_timedwait(&entry->data_available, &entry->mutex, &ts);
      if (rc == ETIMEDOUT) {
        pthread_mutex_unlock(&entry->mutex);
        return 0;
      }
...
void *cache_gc_thread(void *arg) {
  ...
  while (!cache->shutdown) {
    while (!cache->shutdown &&
           cache->total_memory <= cache->max_memory * CACHE_LOW_WATERMARK) {
    ...
    while (cache->total_memory > cache->max_memory * CACHE_LOW_WATERMARK &&
      ...
      if (victim->ref_count > 0 || victim->downloader_active) {
        ...
        cache_entry_t *next_victim = victim->prev;
        int found = 0;
        while (next_victim) {
          pthread_mutex_lock(&next_victim->mutex);
          if (next_victim->ref_count == 0 && !next_victim->downloader_active) {
            pthread_mutex_unlock(&next_victim->mutex);
            victim = next_victim;
            found = 1;
            break;
          }
          pthread_mutex_unlock(&next_victim->mutex);
          next_victim = next_victim->prev;
        }
        ...
        if (!found) {
          ...
          pthread_cond_timedwait(&cache->gc_needed, &cache->mutex, &ts);
          break;
        }
      } else {
        pthread_mutex_unlock(&victim->mutex);
      ...
      size_t entry_size =
          sizeof(cache_entry_t) + strlen(victim->url) + 1 + victim->headers_len;
