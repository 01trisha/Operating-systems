...
Storage* initialize_storage(int capacity) {
    ...
    pthread_spin_init(&storage->head_spinlock, PTHREAD_PROCESS_PRIVATE);
    return storage;
}
...
void add_node(Storage *storage, const char *value) {
    ...
    pthread_spin_lock(&storage->head_spinlock);
    ...
    pthread_spin_unlock(&storage->head_spinlock);
}
...
void free_storage(Storage *storage) {
    ...
    pthread_spin_destroy(&storage->head_spinlock);
    free(storage);
}
